...
from app.persistence.models import ProjectVersion, SavedProject, ShareGrant
...
class ProjectService:
    ...
    def restore_share(self, token: str) -> SharedVersionResponse:
        ...
        return SharedVersionResponse(
            configuration=DesignConfiguration.model_validate(row[1].configuration)
        )
    ...
    def _validate_configuration(self, configuration: DesignConfiguration) -> None:
        DesignService._validate_configuration(configuration)
        if not self._patterns.is_active(configuration.pattern_id):
    ...
    @staticmethod
    def _snapshot(configuration: DesignConfiguration) -> dict[str, object]:
    ...
    def _version_response(
        ...
        return VersionResponse(
            id=version.id,
            version_number=version.version_number,
            configuration=DesignConfiguration.model_validate(version.configuration),
