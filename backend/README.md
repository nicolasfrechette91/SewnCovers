...
- Production refuses to start unless `FRONTEND_ORIGIN` resolves to exactly `https://nicolasfrechette91.github.io`; another valid origin cannot widen or replace the deployed policy.
...
exact `20260829_01` revision, expected tables and named primary/unique/check/
...
python -m ruff check .
python -m pytest
python -m app.assurance.cli readiness
```
