...
export function OrderCard({ order, detail = false }: Readonly<{ order: Order; detail?: boolean }>) {
  ...
  return <article className="min-w-0 wrap-anywhere rounded-panel border border-border bg-surface p-card shadow-card">
...
export function OrdersScreen({ pollPending = false }: Readonly<{ pollPending?: boolean }>) {
  ...
  if (state.status === "guest") return <SignInForCommerce context={pollPending ? "checkout" : "orders"} sessionNotice={state.notice} />;
