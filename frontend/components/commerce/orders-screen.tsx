...
export function OrderCard({ order, detail = false }: Readonly<{ order: Order; detail?: boolean }>) {
  ...
  return <article className="rounded-panel border border-border bg-surface p-card shadow-card">
    ...
    {detail ? <>
      <section className="mt-component"><h3 className="font-display text-section-title font-heading">Configuration snapshot</h3><ul className="mt-3 space-y-2">{order.lines.map((line, index) => <li key={String(line.quoteId ?? index)} className="rounded-card bg-surface-subtle p-3"><span className="font-control">Line {index + 1}</span> · {configurationSummary(line) || "Immutable configured cover"}<br /><span className="text-supporting text-text-muted">Quantity {String(line.quantity ?? "—")} · ${(Number(line.extendedAmountMinor ?? 0) / 100).toFixed(2)} CAD</span></li>)}</ul></section>
...
export function OrdersScreen({ pollPending = false }: Readonly<{ pollPending?: boolean }>) {
  ...
  if (state.status === "initializing" || loading) return <LoadingState label="Checking authoritative order status…" />;
  if (state.status === "guest") return <SignInForCommerce />;
  return <div className="space-y-component"><DemoBanner />{pollPending ? <p className="rounded-card border border-brand bg-surface p-3" role="status">The checkout redirect is informational. This page polls the server; only a verified payment event can change payment status.</p> : null}{error ? <CommerceError message={error} /> : null}{requestedOrder ? <>{selected ? <OrderCard order={selected} detail /> : null}<div className="flex flex-wrap gap-3"><Link href="/orders/" className="inline-flex min-h-11 items-center text-button font-control text-brand underline">All orders</Link><Button variant="secondary" onClick={() => { setLoading(true); void load(state.token); }}>Refresh authoritative status</Button></div></> : orders.length ? <ul className="space-y-component">{orders.map((order) => <li key={order.id}><OrderCard order={order} /></li>)}</ul> : <section className="rounded-panel border border-border bg-surface p-card text-center"><h2 className="font-display text-section-title font-heading">No demonstration orders yet</h2><p className="mt-2 text-text-muted">Paid access is never required for configuring, saving, or sharing.</p><Link href="/commerce/" className="mt-4 inline-flex min-h-11 items-center text-brand underline">View optional demonstration pricing</Link></section>}</div>;
}
