...
- `ShapeSelectionStep` owns the shape-selection presentation while the central configuration Context remains the only source of selection state. It renders a native radio group with whole-card labels, visible checked and availability text, and the documented square, rectangle, and box / bench identifiers.
...
The state follows the documented shared configuration contract: `shape`, `width`, `height`, `thickness`, `unit`, `patternId`, and `patternScale`. Shape, measurements, and pattern selection begin as `null`; the initial unit is centimetres and the documented default pattern scale is `1`. Supported actions set each general field, set square width and mirrored height together with `setSquareWidth`, convert all non-null measurements and change units together with `setMeasurementUnit`, or restore the complete initial state with `resetConfiguration`. Measurement actions ignore non-null values that are not finite and greater than zero. While Square is selected, both `setWidth` and `setSquareWidth` update `width` and `height` together and `setHeight` is ignored, so the invariant cannot be broken through the public reducer actions. Pattern-scale helpers define the `0.5`–`2.0` bounds, `0.1` step, formatting, and normalization; the reducer ignores non-finite and out-of-range scale payloads.

Pure helpers in `context/configuration/measurements.ts` define the centimetre rules, decimal parsing, two-place display formatting, `1 in = 2.54 cm` conversion, range presentation, and shape-completion checks. Width and Height share the 10–300 cm rule; Thickness uses 1–60 cm. The reducer only applies typed immutable transitions; square normalization and conversion are atomic so Context never exposes a newly selected Square with mismatched face dimensions or mixed width, height, thickness, and unit values. The local shape records in `data/shapes.ts` are the single typed owner of the `square`, `rectangle`, and `box` identifiers, names, option descriptions, required visible measurement fields, and Height-versus-Depth terminology. Pattern records remain separate in `data/patterns.ts`.
...
Measurements appear after any supported shape is selected. Square collects Width and Thickness; its committed `height` always mirrors Width. Rectangle collects independent Width, Height, and Thickness values. Box / bench collects Width, Depth, and Thickness, with Depth stored in the shared `height` field exactly as documented. No extra dimension, seam allowance, tolerance, or manufacturing field is collected.
...
The responsive inline SVG measurement guide changes with the selected shape and visibly labels exactly the same terms as its inputs: Width/Thickness for Square, Width/Height/Thickness for Rectangle, and Width/Depth/Thickness for Box / bench. The geometry is decorative and hidden from assistive technology because the visible field labels, descriptions, and figure caption provide equivalent guidance. System-color strokes and fills keep its boundaries visible in forced-colors mode.

## Local pattern catalogue and filters
...
`data/patterns.ts` is the single typed owner of 15 deterministically ordered local patterns. The stable identifiers are `prototype-botanical`, `fern-trail`, `meadow-sprig`, `prototype-geometric`, `diamond-path`, `arch-grid`, `harbor-stripe`, `orchard-stripe`, `ribbon-stripe`, `prototype-woven`, `basket-check`, `linen-crosshatch`, `terrace-wave`, `pebble-drift`, and `confetti-grid`. The original three identifiers, names, descriptions, and preview style handles remain intact.

Each record has one typed category ID, one or more typed color IDs, a visible name, a concise description, and one unique CSS preview-style handle. Category definitions pair the stable `botanical`, `geometric`, `striped`, `woven`, and `abstract` IDs with user-facing labels. Color definitions do the same for `ivory`, `green`, `terracotta`, `blue`, `gold`, `charcoal`, and `rose`. The module also derives the filter types and labels, validates the 12–20 count contract and record completeness, rejects duplicate IDs, case-insensitive names, preview handles, and per-record color tags, resolves selected IDs, and exposes the pure local filter helper. Empty input is represented separately from invalid data so the UI cannot silently render misleading cards.

Category and color each use a visibly labeled native single-choice radio group supplied through `PatternFilter`, including explicit All categories and All colors choices. Category and color combine with AND semantics; a selected color matches any record whose typed color collection includes it. The polite result status reports the visible and total counts. Clear filters restores both All choices and focuses the stable All categories control. Filter values remain local browser-view state in `PatternStep`; only `state.patternId` is configuration state, and it changes only through the existing typed `setPatternId` reducer action.

Filtering never chooses, clears, or replaces a pattern. A filtered-out selection remains in Context and in the shape-aware preview; a named status explains that it is hidden and offers a direct clear-filters action. A selected ID that cannot be resolved is reported without a fallback and can be replaced by choosing any available card. Zero filter matches have a distinct named empty state and clear-filters recovery, while a genuinely empty catalogue has its own named state and preserves the configuration. Invalid or duplicate catalogue records render the existing non-crashing error primitive and suppress misleading cards.
...
All 15 previews are responsive repeating CSS gradients in `app/globals.css`, selected through the record’s typed style handle and reused by the browser card and every shape-aware cushion preview. The `--pattern-scale` value continues to change each motif’s tile or repeat interval. The browser makes no image or network request for these decorative samples, so there is no public pattern URL that can lose the GitHub Pages `/sewncovers` prefix. Equivalent names and descriptions remain visible and the decorative preview containers remain hidden from assistive technology.

Remote catalogue loading, backend replacement, uploads, search, sorting, pagination, advanced faceting, full step navigation, pricing, persistence, cart, and checkout remain deferred.

## Shape-aware proportional preview

`PreviewStep` appears after any shape selection and reads `shape`, `width`, `height`, `thickness`, `unit`, `patternId`, and `patternScale` directly from the central Context. It owns no duplicate configuration state. Valid known pattern selection advances the display-only progress indicator to Preview; no workflow navigation or review behavior is added.

The decorative visual uses a responsive inline SVG with a `360 × 280` view box. The pure `calculatePreviewGeometry` helper validates the selected shape and finite in-range measurements, enforces equal Width/Height for Square, and normalizes valid values to centimetres. Square retains the established horizontal `0.55` and vertical `0.35` thickness projection. Rectangle uses the same face/profile projection with independent proportions. Box / bench uses Width × Depth for its patterned top and deterministic horizontal `0.32` and vertical `0.60` thickness projection for a more top-oriented bench representation. Every shape applies one uniform fit factor to its complete envelope and centers it inside fixed padding, so minimum and maximum valid combinations remain contained without defaults, division by zero, or non-finite coordinates. Equivalent physical centimetre and inch inputs produce equivalent geometry within the established two-decimal conversion tolerance. All projections are illustrative 2D aids, not manufacturing templates, perspective guarantees, fit guarantees, or true 3D rendering.

The selected local record supplies its typed CSS class name; `app/globals.css` remains the only owner of all 15 gradient definitions. The patterned face is rendered through an SVG `foreignObject`, while two solid SVG polygons show the projected side and lower thickness. A safe numeric `--pattern-scale` custom property changes each pattern’s tile or repeat interval. `0.5×` makes the motif half the default size and `2.0×` makes it twice the default size; this visual multiplier is explicitly not presented as a real-world measurement.
