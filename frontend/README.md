...
Future parent components remain responsible for adopting the typed client for design persistence. `Configurator`, `MeasurementStep`, `PatternStep`, `PreviewStep`, and the review components share the completed three-shape flow; the illustration and measurement-diagram components retain shape-specific SVG branches where the geometry or terminology genuinely differs.
...
Saving is available only on the ready review screen. The save boundary maps the authoritative Context state to the exact public `CreateDesignRequest` fields: `shape`, `width`, `height`, `thickness`, `unit`, `patternId`, and `patternScale`. It rechecks supported shape rules, square equality, measurement ranges and precision, normalized API pattern IDs, and pattern-scale range and precision before calling the typed `POST /designs` client. Pattern selection must already resolve through the validated `/patterns` catalogue; no bundled metadata or internal UI fields enter the request.
